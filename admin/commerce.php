		...
		<div class="tab_container">
			...
			<tbody> 
				...
				$stmt->execute(array("id"=>$_SESSION["id"]));
				    ...
				    {
					    $i++;
					    $comm = $bdd->prepare('SELECT * FROM commerce WHERE idcommerce = :id_comm AND commercestatus<>0');
				        $comm->execute(array("id_comm"=>$donnees["compers_commerceID"]));
				        $donnees2=$comm->fetch();
					    echo "<tr>
						<td>".$donnees2["commercenom"]."</td>
						<td>".Status($donnees2["commercestatus"])."</td>
						<td><a href='commerce-gerer.php?edit=".$donnees2["idcommerce"]."' title='pages'><input type='image' src='images/icn_edit.png' title='Edit'></a>
