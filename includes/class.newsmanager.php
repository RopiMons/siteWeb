...
function Lister_news($complet=0,$total,$bdd,$type=""){
	...
	while($donnees=$stmt->fetch())
	{
		...
		if($nb_aff_complet>0) AffichageComplet($news,$bdd);
		else AffichageReduit($news,$type);
		$nb_aff_complet--;
	}
	$stmt->closeCursor();
}
...
function AffichageReduit($news){
	...
	echo '<li><div class="date">
			         ...
			         <p>'.tronque($texte,100).' <a href="single.php?id='.$news["id"].'" class="read-more">En savoir + <i class="icon-angle-right"></p></i></a></li>';
}
